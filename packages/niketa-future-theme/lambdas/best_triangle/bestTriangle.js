...
export function calculateTriangleScore(
  color1,
  color2,
  color3,
  background
...
export async function findBestTriangle({
  colors,
  minBetween = 1.8,
  background,
}){
  ...
  return piped(
    indexList,
    map(indexListInstance =>
      evaluateCombination(indexListInstance, colors, background)
    ),
    filter(Boolean),
